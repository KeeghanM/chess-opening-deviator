...
function Pgn(pgn) {
  ...
  this.pgn = pgn
  this.pgnRaw = pgn
  if (isPGNBroken(pgn)) this.pgnStripped = stripItBroken(pgn)
  else this.pgnStripped = stripIt(pgn)

  /* constructor */

  // strip comments
  if (isPGNBroken(pgn)) this.pgn = stripItBroken(pgn, true)
  else this.pgn = stripIt(pgn, true)
  ...
  for (var i = 0; i < sizeOfTheMoves; i++) {
    ...
    if (c >= "1" && c <= "9") {
      ...
      if (c == ".") {
        // ends with . so nothing but a move
        continue
      }
      var found = false
